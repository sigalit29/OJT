//soucre: http://leaks.wanari.com/2016/12/21/quick-guide-implement-cross-browser-angularjs-pull-refresh-directive/
apple.directive('pullToRefresh', ['$timeout', function ($timeout) {
	return {
		templateUrl: './directives/pull-to-refresh/progressbar.html',
		transclude: true,
		scope: {
			updateFn: '&',
		...
		link: function (scope, element) {
			...
			function onTouchMove(event) {
				var windowHeight = window.innerHeight;
				this.currentMousePosition = event.originalEvent.changedTouches[0].clientY;
				if(!this.reachedTheTop) {
					var allowUp = (document.getElementsByClassName('scrollablecontainer')[0].scrollTop > 1);
					var up = (event.originalEvent.changedTouches[0].pageY > this.slideBeginY);
					var down = (event.originalEvent.changedTouches[0].pageY < this.slideBeginY);
					this.slideBeginY = event.originalEvent.changedTouches[0].pageY;
					if((up && !allowUp) && !down) {
						this.reachedTheTop = true;
						this.refreshStartPosition = this.currentMousePosition;
					}
