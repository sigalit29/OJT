apple.controller('studentProfile', ['$scope', '$rootScope', '$state', '$timeout', 'server', '$filter', '$stateParams', 'Upload',
function($scope, $rootScope, $state, $timeout, server, $filter, $stateParams, Upload) {
	function setTitle() {
		headertitle = $rootScope.userType == 'student'? $rootScope.dictionary.myProfile:$rootScope.dictionary.profileOf+" "+$scope.user.firstname+" "+$scope.user.lastname;
	...
	function getUser() {
		...
		server.requestPhp(data, "GetStudentProfileById").then(function(data) {
			...
			if (data && !data.error) {
				...
				setTitle();
				if($scope.isMobile()&&$rootScope.userType != 'student')
					setContactLinks();
	...
	function setUser() {
		...
		data.id=$stateParams["studentId"];
		server.requestPhp(data, "UpdateStudentProfile").then(function(data) {
			getUser();
		});
	}
...
function updateProfilePic ()
{
	var data = {};
		...
		data.type = "post";
		server.requestPhp(data, "UpdateStudentProfilePic").then(function(data) {
			if($rootScope.userType == 'student')
			{
	...
	$scope.validatePhone2 = function() {
		...
		return validatePhoneNum($scope.user.phone2)||$scope.user.phone2=='';
	};
	...
	$scope.validateAllFields = function() {
		return $scope.validateAddress() && $scope.validateDate() && $scope.validateId() && $scope.validatePhone1() && $scope.validatePhone2();
	};
